                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <c:forEach items="${info.list}" var="p">
                            <tr>
                                ...
                                <td><fmt:formatDate value="${p.odate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                            </tr>
                        </c:forEach>
                    </table>
                    <!--分页栏-->
...
<script type="text/javascript">
    ...
    function del(oid) {
        ...
        if (confirm("您确定删除吗?")) {
            //发出ajax的请求,进行删除操作
            $.ajax({
                url: "${pageContext.request.contextPath}/order/delete.action",
                data: {"oid": oid},
                type: "post",
                dataType: "text",
                success: function (msg) {
                    alert(msg);
                    $("#table").load("http://localhost:9999/admin/order.jsp #table");
                }
            });
        }
    }
...
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/order/ajax2.action",
            data: {"page": page},
            type: "post",
            success: function () {
                //重新加载显示分页数据的容器
                $("#table").load("http://localhost:9999/admin/order2.jsp #table");
            }
        });
    ...
    function condition() {
        ...
        $.ajax({
            type:"post",
            url:"${pageContext.request.contextPath}/order/select.action",
            data:{"uid":uid},
            success:function () {
                //刷新显示数据的容器
                $("#table").load("http://localhost:9999/admin/order.jsp #table");
